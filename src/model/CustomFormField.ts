...
import { CustomFieldValidatorDto, CustomFormFieldDto, CustomFormFieldType, TypeCustomValidation } from '@/dtos';
...
export class CustomFormField extends Entity<CustomFormFieldDto> {
	...
	get isDependent(): boolean {
		return this.dto?.isDependent ?? false;
	}
	...
	get validator(): CustomFieldValidator {
		return new CustomFieldValidator(this?.dto?.validator as CustomFieldValidatorDto);
	}
	...
	rules(customValidation? : TypeCustomValidation) {
		...
		if (this.validator.regX) {
			validations.validator = (value: any) => {
				if (!this.validator.regX?.test(value)) {
